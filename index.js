...
app.post("/task",validateTaskDetails, async(req, res)=>{
    ...
    try {
        const newTask = await client.task.create({
            data:{title: title,description: description, isCompleted:isCompleted}
            
        }) 
        return res.status(201).json({message:"Task created succesfully.", data:{newTask}})       
    } catch (e) {
...
app.patch("/task/:Id", checkId, async(req, res)=>{
    ...
    try {
        const patched = await client.task.update({
            where:{id: Id},
            data:{title: title&&title, description: description&&description, isCompleted: isComplete&&isComplete}
        })
        if (patched) {
